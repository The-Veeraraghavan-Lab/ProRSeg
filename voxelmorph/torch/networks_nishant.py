...
class Unet_All_Encoder_3D_LSTM(nn.Module):
    ...
    def __init__(self, inshape, nb_features=None, nb_levels=None, feat_mult=1):
        ...
        self.conv_lstm1=ConvLSTM3D(input_channel=4, num_filter=2, b_h_w=(1,128,192,128), kernel_size=3,)
...
class VxmDense_3D_LSTM_Step_Reg_All_Encoder_LSTM(LoadableModel):
    ...
    def forward(self, source, source_m,target,target_m,state_h_in,state_c_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        # concatenate inputs and propagate unet
        x = torch.cat([source, target,source_m, target_m], dim=1)
        x,state_h,state_c = self.unet_model(x,state_h_in,state_c_in)
...
class VxmDense_3D_LSTM_Step_Reg_All_Encoder_LSTM_Seg_InPut(LoadableModel):
    """
    VoxelMorph network for (unsupervised) nonlinear registration between two images.
    """
    ...
    def forward(self, source, source_m,target,target_m,state_h_in,state_c_in):
        ...
        #h=None
        #c=None
        # print("Source:\n",torch.unsqueeze(torch.argmax(source_m, dim=1),dim=1).shape)
        # print(torch.unique(target_m))
        # print("Target:\n",target_m.shape)
        # concatenate inputs and propagate unet


        # source_inp_m = torch.zeros_like(source_m)
        # source_inp_m, _ = torch.max(torch.where(source_m>0,1,source_inp_m), dim=1)
        # source_inp_m = torch.unsqueeze(source_inp_m, dim=1)

        source_mask = source_m[:,3,:,:,:]
        source_mask = torch.unsqueeze(source_mask, dim=1)

        target_m[target_m>1] = 1
        
        # print(torch.unique(target_m))
        x = torch.cat([source,  source_mask, target, target_m], dim=1) #here will need to  x = torch.cat([source, source_m, target,target_m], dim=1)
        x,state_h,state_c = self.unet_model(x,state_h_in,state_c_in)
    ...
    def forward_seg_training_all_enc_lstm(self, source, target,source_m,target_m,state_h_in,state_c_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        # concatenate inputs and propagate unet
        # x = torch.cat([source, target], dim=1)
        x = torch.cat([source,  torch.unsqueeze(torch.argmax(source_m, dim=1),dim=1), target, target_m], dim=1)
    ...
    def forward_seg_training_all_enc_lstm_accu_dvf(self, source, target, source_m, target_m,state_h_in,state_c_in,flow_in,plan_ori_img,plan_ori_msk):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        # source_inp_m = source_m.clone()
        # source_inp_m[source_inp_m>1] = 1
        # source_mask = source_m[:,3,:,:,:]
        # source_mask = torch.unsqueeze(source_mask, dim=1)

        # print(source_inp_m.shape)
        target_m[target_m>1] = 1
        
        x = torch.cat([source,  source_m, target, target_m], dim=1) #here will need to  x = torch.cat([source, source_m, target,target_m], dim=1)
        x,state_h,state_c = self.unet_model(x,state_h_in,state_c_in)
