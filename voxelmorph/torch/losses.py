...
class DiceLoss_test_use(nn.Module):
    def __init__(self,num_organ=4):
        super(DiceLoss_test_use, self).__init__()
    ...
    def forward(self, pred_stage1, organ_target):
        ...
        for organ_index in range(0, num_organ ):
            #print ('organ_index is ',organ_index)
...
class mask_NCC:
    ...
    def loss(self, y_true, y_pred, y_pred_msk):
        ...
        #cc_region = torch.where((y_pred_msk!=1),0,cc)

        mask_ncc_region_Jue = (cc * y_pred_msk).sum() / (y_pred_msk.sum() + 1e-5)

        #return -torch.mean(cc), -torch.sum(cc_region) / torch.count_nonzero(cc_region) 
        return -torch.mean(cc), -mask_ncc_region_Jue
